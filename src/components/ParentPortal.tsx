...
function Dashboard({ studentId, onNav }: { studentId: string; onNav: (s: ParentScreen, extra?: string) => void }) {
 ...
 snap.feeStatus === 'paid' ? 'border-zinc-700/50 bg-zinc-800/50' :
 snap.feeStatus === 'due' ? 'border-amber-500/20 bg-amber-500/5' :
 'border-red-500/20 bg-red-500/5'
 }`}>
 <div className="flex items-center justify-between mb-2">
 <DollarSign size={16} className={
 snap.feeStatus === 'paid' ? 'text-zinc-300' :
 ...
 className="rounded-2xl border border-zinc-700/50 bg-zinc-800/50 p-4 text-left hover:bg-zinc-700/50 transition-colors">
 <div className="flex items-center justify-between mb-2">
 <Bell size={16} className="text-zinc-300" />
 {unread > 0 && <span className="text-xs bg-zinc-800 text-white rounded-full w-5 h-5 flex items-center justify-center font-bold">{unread}</span>}
 </div>
 ...
 className="w-full flex items-center justify-between p-4 rounded-2xl bg-zinc-800/50 border border-zinc-700/50 hover:bg-zinc-700/50 transition-colors"
 >
 ...
 <p className="text-zinc-300 font-semibold text-sm">{pendingLeaves.length} request{pendingLeaves.length > 1 ? 's' : ''} need your approval</p>
 <p className="text-zinc-300/60 text-xs">Tap to review and approve/decline</p>
 </div>
 </div>
 ...
 <button onClick={() => onNav('notifications')} className="text-xs text-zinc-300 hover:text-zinc-100">See all</button>
 </div>
 ...
 {!n.read && <div className="w-1.5 h-1.5 rounded-full bg-zinc-800 mt-1.5 shrink-0" />}
 </div>
 );
 })}
 </div>
...
function Messages({ studentId, onThread }: { studentId: string; onThread: (t: MessageThread) => void }) {
 ...
 <div className="w-12 h-12 rounded-xl bg-zinc-900/50 backdrop-blur-md border border-white/[0.08] flex items-center justify-center shrink-0">
 {thread.role === 'warden' ? <Shield size={20} className="text-zinc-300" /> :
 thread.role === 'proctor' ? <GraduationCap size={20} className="text-zinc-300" /> :
 ...
 {thread.unread > 0 && (
 <span className="w-5 h-5 rounded-full bg-zinc-800 text-white text-xs flex items-center justify-center font-bold shrink-0">
 {thread.unread}
 </span>
 )}
 </button>
...
function ThreadDetail({ thread, onBack }: { thread: MessageThread; onBack: () => void }) {
 ...
 {thread.role === 'warden' ? <Shield size={16} className="text-zinc-300" /> : <GraduationCap size={16} className="text-zinc-300" />}
 </div>
 ...
 m.senderRole === 'parent'
 ? 'bg-zinc-800/50 text-white/90 rounded-br-md'
 ...
 className="flex-1 bg-white/[0.05] border border-white/[0.08] rounded-xl px-4 py-2.5 text-sm text-white/90 placeholder:text-white/30 outline-none focus:border-zinc-700/50 transition-colors"
 />
 <button onClick={send} disabled={!input.trim()}
 className="p-2.5 text-zinc-100 text-white rounded-xl hover:opacity-90 disabled:opacity-30 transition-opacity">
 <Send size={16} />
 </button>
 </div>
 </div>
...
function LeaveApprovals({ studentId }: { studentId: string }) {
 ...
 <CheckCircle2 size={28} className="text-zinc-300/40 mb-3" />
 <p className="text-white/40 text-sm">No pending approvals</p>
 <p className="text-white/25 text-xs mt-1">You're all caught up!</p>
 </div>
 )}
 {pending.map(req => (
 <div key={req.id} className="rounded-2xl border border-zinc-700/50 bg-zinc-800/50 p-5 space-y-4">
 ...
 {req.proctorComment && (
 <div className="flex items-start gap-2 p-3 rounded-xl bg-zinc-800/50 border border-zinc-700/50">
 <CheckCircle2 size={13} className="text-zinc-300 mt-0.5 shrink-0" />
 <div>
 <p className="text-zinc-300 text-xs font-medium mb-0.5">Proctor's note</p>
 <p className="text-zinc-300/70 text-xs">{req.proctorComment}</p>
 </div>
 </div>
 )}
 ...
 confirming.action === 'approved' ? 'bg-zinc-800 hover:bg-zinc-700' : 'bg-red-500 hover:bg-red-400'
 }`}>
 ...
 className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-zinc-800 text-white text-sm font-semibold hover:bg-zinc-700 transition-colors">
...
function NotificationsCenter() {
 ...
 {cat}
 {count > 0 && <span className={`w-4 h-4 rounded-full text-xs flex items-center justify-center font-bold ${filter === cat ? 'bg-black text-white' : 'bg-zinc-800 text-white'}`}>{count}</span>}
 </button>
 );
 })}
 </div>
 ...
 {!n.read && <div className="w-2 h-2 rounded-full bg-zinc-800 mt-1.5 shrink-0" />}
 </motion.div>
 );
 })}
 </div>
...
function EmergencyContacts({ studentId }: { studentId: string }) {
 ...
 <div className="p-4 rounded-2xl border border-zinc-700/50 bg-zinc-800/50 flex items-start gap-3">
 <CheckCircle2 size={16} className="text-zinc-300 mt-0.5 shrink-0" />
 ...
 {c.phone && (
 <a href={`tel:${c.phone}`}
 className="flex items-center gap-3 p-3 rounded-xl border border-zinc-700/50 bg-zinc-800/50 hover:bg-zinc-700/50 transition-all group">
 <PhoneCall size={16} className="text-zinc-300 shrink-0" />
 <span className="text-zinc-300 text-sm font-medium">{c.phone}</span>
 <span className="ml-auto text-xs text-zinc-300/50 group-hover:text-zinc-100 transition-colors">Call →</span>
 </a>
 )}
 {c.email && (
...
function ParentSettings({ studentId, onStudentSwitch }: { studentId: string; onStudentSwitch: (id: string) => void }) {
 ...
 <div className="w-14 h-14 rounded-2xl bg-zinc-900/50 backdrop-blur-md border border-zinc-700/50 flex items-center justify-center">
 ...
 sid === studentId ? 'border-zinc-700/50 bg-zinc-800/50' : 'border-white/[0.05] bg-white/[0.02] hover:bg-white/[0.05]'
 }`}>
 ...
 {sid === studentId && <span className="text-xs text-zinc-300 font-medium">Active</span>}
...
export default function ParentPortal() {
 ...
 <item.icon size={20} />
 {badge > 0 && (
 <span className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-zinc-800 rounded-full text-white text-[10px] flex items-center justify-center font-bold">
 {badge}
 </span>
 )}
 </div>
