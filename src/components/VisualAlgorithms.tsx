...
export default function VisualAlgorithms() {
 ...
 ? 'bg-zinc-900/50 backdrop-blur-md text-white border border-white/10'
 ...
 className="px-4 py-2 text-zinc-100 rounded-lg text-white font-medium hover:opacity-90 transition-all"
 >
 ▶ Run
 </button>
 )}
 ...
 <span className="text-white/60">Comparisons: <span className="text-zinc-300">{sortComparisons}</span></span>
 <span className="text-white/60">Swaps: <span className="text-zinc-300">{sortSwaps}</span></span>
 </div>
 ...
 ? 'bg-yellow-300 shadow-[0_0_10px_rgba(253,224,71,0.5)]'
 : bar.state === 'sorted'
 ? 'bg-amber-500'
 : bar.state === 'pivot'
 ? 'bg-red-400'
 ...
 {graphMessage && (
 <div className="px-4 py-2 bg-white/[0.05] border border-white/10 rounded-lg text-zinc-300 text-sm">
 {graphMessage}
 </div>
 )}
 ...
 disabled={!bstRoot}
 className="px-4 py-2 text-zinc-100 rounded-lg text-white font-medium hover:opacity-90 transition-all disabled:opacity-50"
 >
 ▶ Traverse
 </button>
 )}
 ...
 <span className="text-zinc-300">{bstTraversal.join(' → ')}</span>
 </div>
 )}
