...
export default function AttendanceTracker() {
 ...
 className="w-14 h-14 rounded-2xl text-zinc-100 flex items-center justify-center shadow-lg shadow-emerald-500/30 border border-white/10">
 <BarChart3 size={28} className="text-white" />
 </motion.div>
 <div>
 <h2 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-white to-white/60">Attendance & CGPA</h2>
 ...
 className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeView === v.id ? 'bg-zinc-800/50 text-zinc-300' : 'text-white/40 hover:text-white/80'}`}>
 {v.label}
 </button>
 ))}
 </div>
 ...
 className="px-4 py-2.5 text-zinc-100 text-white font-semibold rounded-xl flex items-center gap-2 shadow-lg shadow-emerald-500/20">
 ...
 className="p-4 bg-zinc-800/50 border border-zinc-700/50 rounded-2xl flex items-center gap-3 flex-shrink-0">
 <AlertTriangle size={20} className="text-zinc-300 flex-shrink-0" />
 <div>
 <p className="text-zinc-300 text-sm font-semibold">Attendance below 75%</p>
 <p className="text-zinc-300/60 text-xs">{dangerSubjects.map(s => s.code).join(', ')} — You may face debarment!</p>
 </div>
 </motion.div>
 )}
 ...
 className="w-full bg-black/30 border border-white/[0.08] rounded-xl px-4 py-3 text-white text-sm outline-none focus:border-zinc-700/50" />
 ...
 <button onClick={addSubject} className="mt-3 w-full py-3 text-zinc-100 text-white font-semibold rounded-xl hover:opacity-90 transition-all">
 Add Subject
 </button>
 </motion.div>
 )}
 ...
 className="px-3 py-1.5 bg-zinc-800/50 text-zinc-300 rounded-lg text-xs font-medium hover:bg-zinc-700/50 transition-all flex items-center gap-1">
 ...
 {pct >= 75 ? (
 <p className="text-zinc-300/70 text-[11px]">✨ You can miss <b>{missable}</b> more classes safely</p>
 ) : (
 <p className="text-zinc-300/70 text-[11px]">⚠️ Attend <b>{needed}</b> consecutive classes to reach 75%</p>
 )}
 </div>
 </div>
 </div>
 ...
 className="bg-zinc-900/50 backdrop-blur-md backdrop-blur-xl border border-zinc-700/50 rounded-3xl p-8 text-center">
 <GraduationCap size={40} className="text-zinc-300 mx-auto mb-4" />
 <p className="text-white/40 text-sm mb-2">Your Current CGPA</p>
 <p className="text-6xl font-black text-zinc-100 ">{cgpa}</p>
 ...
 <p className="text-lg font-bold text-zinc-300">{Math.round(cgpa * 10)}%</p>
 </div>
 </div>
 </motion.div>
 ...
 <p className="text-lg font-bold text-zinc-300">{g.grade}</p>
