...
function RouteList({ onSelect }: { onSelect: (r: BusRoute) => void }) {
 ...
 ann.type === 'warning' ? 'bg-amber-500/10 border-amber-500/20 text-amber-300' : 'bg-zinc-800/50 border-zinc-700/50 text-zinc-300'
 }`}>
 ...
 <div className="w-10 h-10 rounded-xl bg-zinc-800/50 text-zinc-300 flex items-center justify-center font-black text-lg border border-zinc-700/50">
 {route.routeNumber}
 </div>
 <div>
...
function RouteDetail({ route, onBack }: { route: BusRoute, onBack: () => void }) {
 ...
 <div className="w-12 h-12 rounded-xl bg-zinc-800/50 text-zinc-300 flex items-center justify-center font-black text-xl border border-zinc-700/50">
 {route.routeNumber}
 </div>
 <div>
 ...
 <span className="text-zinc-300 flex items-center gap-1"><Clock size={12} /> {stop.expectedTimeMorn} (Pickup)</span>
 <span className="text-zinc-300 flex items-center gap-1"><Clock size={12} /> {stop.expectedTimeEve} (Drop)</span>
 </div>
 </div>
 ))}
 </div>
...
function LiveTracking() {
 ...
 switch(status) {
 case 'on_time': return 'bg-zinc-800/50 text-zinc-300 border-zinc-700/50';
 ...
 return (
 <div className="space-y-6">
 <div className="p-4 rounded-2xl border border-zinc-700/50 bg-zinc-800/50 flex items-start gap-3 text-zinc-300">
 ...
 <Bus size={32} className={route.status === 'delayed' ? 'text-red-400' : 'text-zinc-300'} />
...
function DigitalPass() {
 ...
 {/* Glow Effects */}
 <div className="absolute -top-32 -right-32 w-64 h-64 bg-zinc-800/50 rounded-full blur-3xl" />
 <div className="absolute -bottom-32 -left-32 w-64 h-64 bg-zinc-800/50 rounded-full blur-3xl" />
 ...
 <Bus size={20} className="text-zinc-300" />
 ...
 <p className="text-zinc-300 text-3xl font-black">{MY_BUS_PASS.routeAssigned}</p>
 </div>
