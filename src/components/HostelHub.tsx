...
function HostelCard({ hostel, onClick }: { hostel: Hostel; onClick: () => void }) {
 ...
 <span className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-zinc-800/50 text-zinc-300 border border-zinc-700/50">
...
function HostelDetail({ hostel, onBack, onAdminClick, onComplaintsClick }: { hostel: Hostel; onBack: () => void; onAdminClick: () => void; onComplaintsClick: () => void }) {
 ...
 <Wind size={18} className="text-zinc-300 shrink-0" />
 <p className="text-zinc-300 text-sm font-medium">This block is fully air-conditioned across all room types.</p>
 </div>
 ) : null}
 ...
 {room.ac ? (
 <span className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-zinc-800/50 text-zinc-300">
 <Wind size={9} /> AC
 </span>
 ) : (
...
function CentralAdminScreen({ onBack }: { onBack: () => void }) {
 ...
 'Deputy': <Shield size={16} className="text-zinc-300" />,
 'Food': <Utensils size={16} className="text-green-400" />,
 'Discipline': <BookOpen size={16} className="text-zinc-300" />,
 'Maintenance': <Wrench size={16} className="text-zinc-300" />,
 };
