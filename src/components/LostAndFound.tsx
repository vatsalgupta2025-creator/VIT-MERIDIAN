...
export default function LostAndFound() {
 ...
 <div className="absolute top-3 right-3 px-2 py-1 rounded-lg text-[10px] font-bold bg-zinc-800/50 text-zinc-300 z-10">
 Resolved
 </div>
 )}
 ...
 <span className="text-xs text-zinc-300">Found</span>
 <p className="text-lg font-bold text-white">{stats.found}</p>
 </div>
 <div className="px-4 py-2 rounded-xl bg-zinc-800/50 border border-zinc-700/50">
 <span className="text-xs text-zinc-300">Resolved</span>
 ...
 { id: 'found', label: 'Found', color: 'text-zinc-300' }
 ].map(f => (
 <button
 key={f.id}
 ...
 <div className="flex items-center gap-3">
 <div className="w-10 h-10 rounded-full text-zinc-100 flex items-center justify-center text-sm font-bold text-white">
 ...
 <p className="text-xs text-zinc-300/60 mt-1">{selectedItem.resolvedBy}</p>
 )}
 </div>
 )}
 ...
 className="flex-1 py-3 rounded-xl bg-zinc-800/50 text-zinc-300 font-medium text-sm hover:bg-zinc-700/50 transition-colors"
 >
 Mark as Resolved
 </button>
