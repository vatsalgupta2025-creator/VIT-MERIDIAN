...
export default function Calendar() {
 ...
 ${isToday ? 'bg-zinc-800/50 text-zinc-300 font-bold border border-zinc-700/50' : 'hover:bg-white/10 text-white/70'}
 `}
 >
 <span className="text-sm">{i}</span>
 <div className="flex gap-1 mt-1">
 {hasEvent && <div className={`w-1 h-1 rounded-full ${isCritical ? 'bg-red-400' : 'bg-zinc-800'}`} />}
 </div>
 </motion.div>
 );
 }
