...
export default function TimetableHelper() {
 ...
 {/* Hero */}
 <div className="bg-zinc-900/50 backdrop-blur-md rounded-2xl p-8 border border-white/[0.06]">
 <div className="flex items-center gap-4 mb-4">
 <div className="w-14 h-14 rounded-2xl text-zinc-100 flex items-center justify-center">
 ...
 className="px-6 py-3 text-zinc-100 text-white font-medium rounded-xl flex items-center gap-2 hover:opacity-90 transition-opacity shadow-lg shadow-cyan-500/20"
 >
 ...
 <Upload size={18} className="text-zinc-300" /> Upload Your Timetable
 </h3>
 <p className="text-white/40 text-sm mb-4">
 Paste your timetable data below. We&apos;ll automatically parse the course codes and slots.
 <br />
 <span className="text-zinc-300/60">Format: A1-BACSE105-ETH-AB1-401-ALL</span>
 </p>
 <textarea
 value={uploadText}
 onChange={(e) => setUploadText(e.target.value)}
 placeholder="Paste your timetable here..."
 className="w-full h-64 bg-white/[0.03] border border-white/[0.06] rounded-xl p-4 text-sm text-white/80 placeholder:text-white/20 focus:outline-none focus:border-zinc-700/50 resize-none mb-4"
 />
 <div className="flex gap-3">
 <button
 onClick={handleUpload}
 disabled={!uploadText.trim()}
 className="flex-1 py-3 text-zinc-100 text-white font-medium rounded-xl hover:opacity-90 disabled:opacity-30 transition-opacity"
 >
 Parse Timetable
 </button>
 <button
 ...
 <div className="w-10 h-10 rounded-xl text-zinc-100 flex items-center justify-center">
 ...
 className="px-3 py-2 bg-white/[0.05] text-zinc-300 rounded-lg text-xs hover:bg-zinc-700/50 transition-colors flex items-center gap-1.5"
 >
 ...
 activeTab === tab.id
 ? 'bg-zinc-800/50 text-zinc-300 shadow-sm'
 ...
 {/* Current Class Card */}
 <div className="bg-zinc-900/50 backdrop-blur-md rounded-2xl p-6 border border-white/[0.06]">
 {currentClass?.course ? (
 <div>
 <div className="flex items-center gap-2 mb-3">
 <span className="px-2 py-0.5 bg-zinc-800/50 text-zinc-300 text-[10px] font-medium rounded-full flex items-center gap-1">
 <div className="w-1.5 h-1.5 rounded-full bg-zinc-800 animate-pulse" /> Ongoing
 </span>
 ...
 <MapPin size={14} className="text-zinc-300" /> {currentClass.course.room}
 </span>
 <span className="flex items-center gap-1.5 text-white/60">
 <User size={14} className="text-zinc-300" /> {currentClass.course.faculty}
 </span>
 </div>
 ...
 <motion.div
 className="h-full text-zinc-100"
 initial={{ width: '100%' }}
 ...
 <p className="text-zinc-300/60 text-xs">in {formatTime(currentClass.timeRemaining)}</p>
 </div>
 )}
 </div>
 ) : (
 ...
 <List size={14} className="text-zinc-300" /> Today&apos;s Schedule
 </h3>
 ...
 isCurrent ? 'bg-zinc-800/50 border border-zinc-700/50' :
 isPast ? 'bg-white/[0.02] opacity-50' : 'bg-white/[0.03]'
 }`}
 >
 <div className="w-14 text-center">
 <p className={`text-xs font-medium ${isCurrent ? 'text-zinc-300' : 'text-white/40'}`}>{slot.code}</p>
 ...
 {isCurrent && (
 <span className="px-2 py-0.5 bg-zinc-800/50 text-zinc-300 text-[10px] rounded-full">Now</span>
 )}
 </div>
 );
 })}
 ...
 <p className={`text-xs font-medium ${course ? 'text-zinc-300' : 'text-white/20'}`}>{slot.code}</p>
 ...
 className="text-white/20 hover:text-zinc-100 transition-colors"
 >
 <X size={12} />
 </button>
 </div>
 ...
 className="text-zinc-300/40 hover:text-zinc-100 text-xs flex items-center gap-1"
 >
 <Plus size={12} /> Add
 </button>
 </div>
 ...
 <p className="text-zinc-300/60 text-xs">{course.code}</p>
 </div>
 <span className={`text-[10px] px-1.5 py-0.5 rounded ${course.type === 'THEORY' ? 'bg-zinc-800/50 text-zinc-300' : 'bg-amber-500/15 text-amber-400'}`}>
 ...
 <div className="bg-[#0c0f17] rounded-xl p-5 border border-white/[0.05]">
 <h3 className="text-sm font-semibold text-white mb-4 flex items-center gap-2">
 <Search size={14} className="text-zinc-300" /> Check Slot Availability
 </h3>
 ...
 className="flex-1 bg-white/[0.03] border border-white/[0.06] rounded-lg px-3 py-2 text-sm text-white/80 placeholder:text-white/20 focus:outline-none focus:border-zinc-700/50"
 />
 <button
 onClick={() => setConflicts(checkConflicts(ffcsQuery))}
 className="px-4 py-2 bg-zinc-800/50 text-zinc-300 rounded-lg text-sm hover:bg-zinc-700/50 transition-colors"
 >
 Check
 </button>
 </div>
 ...
 <p className="text-zinc-300 text-xs font-medium mb-1 flex items-center gap-1">
 ...
 <span key={slot} className="px-1.5 py-0.5 bg-zinc-800/50 text-zinc-300 text-[10px] rounded">
 {slot}
 </span>
 ))}
 {free.length > 8 && (
