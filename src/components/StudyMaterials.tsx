...
export default function StudyMaterials() {
 ...
 <div className="absolute -top-20 -right-20 w-64 h-64 bg-zinc-800/[0.08] rounded-full blur-[80px]" />
 <div className="absolute -bottom-16 -left-16 w-48 h-48 bg-zinc-800/[0.06] rounded-full blur-[60px]" />
 <div className="relative z-10 flex items-center justify-between">
 <div>
 <div className="flex items-center gap-3 mb-3">
 <div className="w-11 h-11 rounded-xl text-zinc-100 flex items-center justify-center shadow-lg shadow-cyan-500/25">
 ...
 <span className="text-zinc-300/70 text-[10px] font-medium">{resources.length} Resources</span>
 </div>
 <div className="flex items-center gap-1.5">
 ...
 <Search size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-zinc-300/50 transition-colors" />
 <input
 type="text"
 value={searchQuery}
 onChange={(e) => setSearchQuery(e.target.value)}
 placeholder="Search resources, tags, keywords..."
 className="w-full bg-white/[0.03] border border-white/[0.06] rounded-xl pl-10 pr-4 py-2.5 text-sm text-white/80 placeholder:text-white/15 focus:outline-none focus:border-zinc-700/50 focus:bg-white/[0.04] transition-all"
 />
 </div>
 ...
 ? 'bg-zinc-800/50 text-zinc-300 shadow-sm shadow-cyan-500/10'
 ...
 {/* Glow on hover */}
 <div className={`absolute -top-16 -right-16 w-40 h-40 rounded-full blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 ${r.color === 'cyan' ? 'bg-zinc-800/50' : r.color === 'violet' ? 'bg-zinc-800/50' : 'bg-zinc-800/50'}`} />

 <div className="relative z-10 p-5">
 {/* Header */}
 <div className="flex items-center justify-between mb-4">
 <div className={`w-11 h-11 rounded-xl ${r.color === 'cyan' ? 'bg-zinc-800/50' : r.color === 'violet' ? 'bg-zinc-800/50' : 'bg-zinc-800/50'} flex items-center justify-center group-hover:scale-110 transition-transform`}>
 <r.icon size={20} className={`${r.color === 'cyan' ? 'text-zinc-300' : r.color === 'violet' ? 'text-zinc-300' : 'text-zinc-300'}`} />
 </div>
 ...
 {r.stats && (
 <span className={`text-[9px] font-bold ${r.color === 'cyan' ? 'text-zinc-300/50' : r.color === 'violet' ? 'text-zinc-300/50' : 'text-zinc-300/50'}`}>
 {r.stats}
 </span>
 )}
 </div>
 ...
 <BookMarked size={13} className="text-zinc-300/60" />
 ...
 >
 {/* Icon */}
 <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 transition-all group-hover:scale-110 ${r.color === 'cyan' ? 'bg-zinc-800/50' :
 r.color === 'violet' ? 'bg-zinc-800/50' :
 r.color === 'amber' ? 'bg-amber-500/15' :
 r.color === 'emerald' ? 'bg-zinc-800/50' :
 'bg-zinc-800/50'}`}>
 <r.icon size={20} className={`${r.color === 'cyan' ? 'text-zinc-300' :
 r.color === 'violet' ? 'text-zinc-300' :
 r.color === 'amber' ? 'text-amber-400' :
 r.color === 'emerald' ? 'text-zinc-300' :
 'text-zinc-300'}`} />
 </div>
 ...
 {r.stats && (
 <span className={`px-2.5 py-1 rounded-lg text-[10px] font-semibold ${r.color === 'cyan' ? 'bg-zinc-800/50 text-zinc-300/70' :
 r.color === 'violet' ? 'bg-zinc-800/50 text-zinc-300/70' :
 r.color === 'amber' ? 'bg-amber-500/10 text-amber-400/70' :
 r.color === 'emerald' ? 'bg-zinc-800/50 text-zinc-300/70' :
 'bg-zinc-800/50 text-zinc-300/70'}`}>
 {r.stats}
 </span>
 )}
 ...
 {/* ── Exam Prep Tips ── */}
 <motion.div variants={itemVariants} className="bg-zinc-900/50 backdrop-blur-md rounded-2xl p-6 border border-zinc-800/50 relative overflow-hidden">
 <div className="absolute -bottom-12 -right-12 w-40 h-40 bg-zinc-800/20 rounded-full blur-3xl" />
 ...
 <div className={`w-8 h-8 rounded-lg ${tip.color === 'violet' ? 'bg-zinc-800/50' : tip.color === 'cyan' ? 'bg-zinc-800/50' : 'bg-zinc-800/50'} flex items-center justify-center`}>
 <tip.icon size={14} className={`${tip.color === 'violet' ? 'text-zinc-300' : tip.color === 'cyan' ? 'text-zinc-300' : 'text-zinc-300'}`} />
 </div>
