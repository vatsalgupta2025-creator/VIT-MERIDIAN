...
export default function StudyBuddy() {
 ...
 className="w-14 h-14 rounded-2xl text-zinc-100 flex items-center justify-center shadow-lg shadow-cyan-500/30 border border-white/10"
 >
 <BookOpenCheck size={28} className="text-white" />
 </motion.div>
 <div>
 <h2 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-white to-white/60">AI Study Buddy</h2>
 ...
 className="w-full bg-black/30 border border-white/[0.08] rounded-xl p-4 text-sm text-white/90 placeholder:text-white/20 focus:outline-none focus:border-zinc-700/50 resize-none transition-all"
 rows={3}
 />
 ...
 className="flex-1 py-3 text-zinc-100 text-white font-semibold rounded-xl flex items-center justify-center gap-2 hover:opacity-90 disabled:opacity-30 transition-all shadow-lg shadow-cyan-500/20"
 >
 ...
 <span className="text-zinc-300 text-xs font-bold">{correctCount} âœ“</span>
 </div>
 ...
 {/* Front */}
 <div
 className="absolute inset-0 bg-zinc-900/50 backdrop-blur-md backdrop-blur-xl border border-zinc-700/50 rounded-3xl p-8 flex flex-col items-center justify-center text-center"
 style={{ backfaceVisibility: 'hidden' }}
 >
 <span className="px-3 py-1 bg-zinc-800/50 text-zinc-300 text-[10px] font-bold rounded-full mb-4">QUESTION Â· Card {currentIndex + 1}/{cards.length}</span>
 ...
 >
 <span className="px-3 py-1 bg-zinc-800/50 text-zinc-300 text-[10px] font-bold rounded-full mb-4">ANSWER</span>
 ...
 className="px-5 py-3 bg-zinc-800/50 text-zinc-300 rounded-xl hover:bg-zinc-700/50 transition-all flex items-center gap-2">
 ...
 <button onClick={() => loadDeck(deck)} className="p-1.5 text-white/40 hover:text-zinc-100"><RefreshCw size={12} /></button>
 ...
 <>
 <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-zinc-900/50 backdrop-blur-md backdrop-blur-xl border border-zinc-700/50 rounded-2xl p-6">
 <h3 className="text-xl font-bold text-white mb-1">{summary.title}</h3>
 <div className="flex gap-2 mt-3 flex-wrap">
 {summary.keywords.map((kw, i) => (
 <span key={i} className="px-2.5 py-1 bg-zinc-800/50 text-zinc-300 text-[10px] rounded-full font-medium">{kw}</span>
 ))}
 </div>
 </motion.div>
 ...
 <div className="w-7 h-7 bg-zinc-800/50 text-zinc-300 rounded-lg flex items-center justify-center text-xs font-bold flex-shrink-0">{i + 1}</div>
 ...
 <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-zinc-900/50 backdrop-blur-md backdrop-blur-xl border border-zinc-700/50 rounded-3xl p-10 text-center">
 <Award size={64} className="text-zinc-300 mx-auto mb-4" />
 <h3 className="text-3xl font-bold text-white mb-2">Quiz Complete!</h3>
 <p className="text-5xl font-black text-zinc-100 my-4">{quizScore}/{quizQuestions.length}</p>
 ...
 className="px-6 py-3 bg-zinc-800/50 text-zinc-300 rounded-xl hover:bg-zinc-700/50 transition-all flex items-center gap-2 mx-auto">
 ...
 <div key={i} className={`flex-1 h-1.5 rounded-full ${i < quizIndex ? 'bg-zinc-800' : i === quizIndex ? 'bg-zinc-800/50' : 'bg-white/[0.06]'}`} />
 ))}
 </div>

 {/* Question */}
 <div className="bg-white/[0.03] backdrop-blur-xl border border-white/[0.06] rounded-2xl p-6">
 <span className="text-zinc-300 text-xs font-bold">Question {quizIndex + 1} of {quizQuestions.length}</span>
 ...
 ? 'bg-white/[0.03] border border-white/[0.06] hover:border-zinc-600/50 text-white/80'
 ...
 <button onClick={nextQuizQuestion} className="mt-3 px-5 py-2 bg-zinc-800/50 text-zinc-300 rounded-lg hover:bg-zinc-700/50 text-sm transition-all">
