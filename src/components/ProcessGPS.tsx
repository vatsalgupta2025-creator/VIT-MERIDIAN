...
function StageIcon({ status }: { status: WorkflowStage['status'] }) {
 switch (status) {
 case 'completed':
 return <CheckCircle2 size={22} className="text-zinc-300" />;
 case 'in-progress':
 return (
...
function statusBadge(status: WorkflowStage['status']) {
 ...
 case 'completed':
 return (
 <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-zinc-800/50 text-zinc-300 uppercase tracking-wider">
 Completed
 </span>
 );
 case 'in-progress':
 return (
 <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-zinc-800/50 text-zinc-300 uppercase tracking-wider animate-pulse">
 In Progress
 </span>
 );
 case 'pending':
...
export default function ProcessGPS() {
 ...
 <p className="text-2xl font-bold text-zinc-300">{Math.round(progressPct)}%</p>
 ...
 <p className="text-[11px] text-zinc-300/60 font-mono">
