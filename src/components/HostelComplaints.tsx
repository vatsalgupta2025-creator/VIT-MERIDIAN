...
function RaiseComplaint({ onBack, onSubmit }: { onBack: () => void, onSubmit: (c: Complaint) => void }) {
 ...
 <button onClick={() => setStep(1)} className="ml-auto text-xs text-zinc-300 hover:text-zinc-100">Change</button>
 </div>

 {/* Sensitive Notice */}
 {catConfig.sensitiveNote && (
 <div className={`p-4 rounded-xl border flex gap-3 ${isSensitive ? 'bg-zinc-800/50 border-zinc-700/50 text-zinc-300' : 'bg-amber-500/10 border-amber-500/20 text-amber-300'}`}>
 ...
 className={`relative w-12 h-6 rounded-full transition-colors ${anon ? 'bg-zinc-800' : 'bg-white/10'} ${isSensitive ? 'opacity-50 cursor-not-allowed' : ''}`}
 >
...
function ComplaintDetail({ complaint, onBack }: { complaint: Complaint, onBack: () => void }) {
 ...
 <><Shield size={14} className="text-zinc-300" /><span className="text-white/70 text-sm">Anonymous</span></>
 ) : (
 ...
 c.authorRole === 'student' || c.authorRole === 'anonymous_student' ? 'bg-zinc-800/50 text-white border border-zinc-700/50' : 'bg-white/[0.05] text-white/80 border border-white/[0.08]'
 }`}>
 {c.body}
 </div>
 ...
 <div className="p-5 rounded-2xl border border-zinc-700/50 bg-zinc-800/50 text-center">
 <p className="text-zinc-300 font-semibold mb-1">Issue Resolved?</p>
 <p className="text-zinc-300/60 text-sm mb-4">Let us know if you're satisfied with the resolution.</p>
 <div className="flex justify-center gap-3">
 <button className="flex items-center gap-2 px-4 py-2 rounded-xl bg-zinc-800/50 border border-zinc-700/50 text-zinc-300 hover:bg-zinc-700/50 transition-colors">
