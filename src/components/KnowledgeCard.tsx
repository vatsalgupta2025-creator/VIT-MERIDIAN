...
function Toast({ message, onClose }: { message: string; onClose: () => void }) {
 ...
 return (
 <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[60] flex items-center gap-2 px-4 py-2.5 rounded-xl bg-zinc-800 text-white text-sm font-medium shadow-lg shadow-emerald-500/30 animate-in-slide">
 <CheckCircle size={16} />
 {message}
 <style jsx>{`
 .animate-in-slide {
...
const typeConfig: Record<string, { icon: React.ReactNode; color: string; bg: string }> = {
 ...
 VIDEO: { icon: <Video size={20} />, color: 'text-zinc-300', bg: 'bg-zinc-800/50' },
 DOCX: { icon: <FileText size={20} />, color: 'text-zinc-300', bg: 'bg-zinc-800/50' },
 CODE: { icon: <Code size={20} />, color: 'text-zinc-300', bg: 'bg-zinc-800/50' },
 NOTES: { icon: <BookOpen size={20} />, color: 'text-zinc-300', bg: 'bg-zinc-800/50' },
};
...
export default function KnowledgeCard({ resource, onClose }: KnowledgeCardProps) {
 ...
 <div className="w-5 h-5 rounded-md text-zinc-100 flex items-center justify-center">
 ...
 {isLoading && (
 <span className="text-[11px] text-zinc-300/60 animate-pulse ml-2">
 Extracting text...
 </span>
 )}
 </div>
 ...
 {displayedText}
 {isTyping && (
 <span className="inline-block w-[2px] h-4 bg-zinc-800 ml-0.5 animate-pulse align-middle" />
 )}
 </p>
 )}
 </div>
 ...
 <button 
 onClick={handleView}
 className="flex items-center gap-2 px-4 py-2 rounded-xl text-zinc-100 text-white text-sm font-medium hover:opacity-90 transition-opacity shadow-lg shadow-cyan-500/20"
 >
 ...
 <button className="w-full flex items-center justify-between p-3 rounded-xl bg-zinc-800/50 border border-zinc-700/50 text-zinc-300/70 hover:text-zinc-100 transition-colors group">
