...
function FacultyCard({ faculty, onClick }: { faculty: FacultyDetail; onClick: () => void }) {
    ...
    return (
        <motion.div
            ...
            onClick={onClick}
            className="group relative bg-white/[0.03] border border-white/[0.07] rounded-2xl p-5 cursor-pointer hover:border-cyan-500/30 hover:bg-white/[0.06] transition-all duration-200"
        >
            <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-cyan-500/0 to-violet-500/0 group-hover:from-cyan-500/[0.04] group-hover:to-violet-500/[0.04] transition-all duration-300 pointer-events-none" />
            <div className="flex items-start gap-4">
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-violet-600 to-cyan-500 flex items-center justify-center text-white font-bold text-lg flex-shrink-0 shadow-lg shadow-violet-500/20 group-hover:shadow-cyan-500/20 transition-shadow">
                    {initials}
                </div>
                <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                        <div>
                            ...
                            <p className="text-xs text-violet-400/80 font-medium mt-0.5">{faculty.designation}</p>
                        </div>
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2">
                        <span className="flex items-center gap-1 text-xs text-white/50">
                            <Building2 size={10} className="text-cyan-400/60" />
                            {faculty.school}
                        </span>
                        <span className="flex items-center gap-1 text-xs text-white/50">
                            <GraduationCap size={10} className="text-cyan-400/60" />
                ...
                <ChevronRight size={16} className="text-white/20 group-hover:text-cyan-400 flex-shrink-0 mt-1 transition-colors" />
            </div>
...
function FacultyDetailModal({ faculty, onClose }: { faculty: FacultyDetail; onClose: () => void }) {
    ...
    return (
        ...
        >
            ...
            >
                <div className="h-24 bg-gradient-to-br from-violet-600/40 to-cyan-600/30 relative">
                    <button
                        onClick={onClose}
                    ...
                    <div className="absolute -bottom-8 left-6">
                        <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-violet-600 to-cyan-500 flex items-center justify-center text-white font-bold text-2xl shadow-2xl shadow-violet-500/30 border-4 border-[#0a0f1c]">
                            {initials}
                        </div>
                    </div>
                </div>
                <div className="pt-12 px-6 pb-6 overflow-y-auto max-h-[70vh]">
                    <div className="flex items-start justify-between">
                        <div>
                            ...
                            <p className="text-sm text-violet-400 font-medium mt-0.5">{faculty.designation}</p>
                            {faculty.rating && (
                    ...
                    <div className="flex flex-wrap gap-2 mt-4">
                        <span className="flex items-center gap-1.5 text-xs px-3 py-1 rounded-full bg-cyan-500/10 text-cyan-400 border border-cyan-500/20">
                            <Building2 size={10} />{faculty.school}
                        </span>
                        <span className="flex items-center gap-1.5 text-xs px-3 py-1 rounded-full bg-violet-500/10 text-violet-400 border border-violet-500/20">
                        ...
                        <div className="mt-4">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {faculty.researchInterests.map((interest) => (
                                    <span
                                        key={interest}
                                        className="text-xs px-3 py-1 rounded-full bg-white/[0.04] text-white/60 border border-white/[0.07] hover:border-cyan-500/30 hover:text-cyan-400 transition-colors"
                                    >
                                        {interest}
                                    </span>
                                ))}
                    ...
                    <div className="mt-5 flex flex-col gap-3">
                        <motion.a
                            ...
                            whileTap={{ scale: 0.98 }}
                            className="flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-gradient-to-r from-cyan-600 to-violet-600 text-white text-sm font-semibold shadow-lg shadow-cyan-500/20 hover:shadow-cyan-500/30 transition-shadow"
                        >
...
function LoginScreen({ onLogin }: { onLogin: (auth: AuthState) => void }) {
    ...
    return (
        <div className="min-h-full flex flex-col items-center justify-center py-12 px-4">
            <div className="fixed top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-violet-600/5 rounded-full blur-[120px] pointer-events-none" />
            <motion.div
            ...
            >
                <div className="relative bg-white/[0.03] backdrop-blur-xl border border-white/[0.08] rounded-3xl p-8 shadow-2xl overflow-hidden">
                    ...
                    <div className="text-center mb-8">
                        <div className="w-16 h-16 mx-auto rounded-2xl bg-gradient-to-br from-cyan-500 to-violet-600 flex items-center justify-center shadow-xl shadow-cyan-500/20 mb-4">
                    ...
                    <div className="mb-4">
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full bg-white/[0.05] border border-white/[0.08] focus:border-cyan-500/50 rounded-xl pl-10 pr-4 py-3 text-sm text-white placeholder-white/25 outline-none transition-colors focus:bg-white/[0.07]"
                            />
                        </div>
                    </div>
                    <div className="mb-6">
                    ...
                    <motion.button
                        ...
                        onClick={handleLogin}
                        disabled={loading}
                        className="w-full py-3.5 rounded-xl bg-gradient-to-r from-cyan-600 to-violet-600 text-white font-semibold text-sm shadow-xl shadow-cyan-500/20 hover:shadow-cyan-500/30 transition-shadow disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                    >
                        {loading ? (
    ...
    return (
        <div className="space-y-6">
            ...
            <motion.a
                ...
                whileTap={{ scale: 0.99 }}
                className="flex items-center justify-between gap-4 px-5 py-3.5 rounded-2xl bg-gradient-to-r from-cyan-500/10 to-violet-500/10 border border-cyan-500/20 hover:border-cyan-500/40 hover:from-cyan-500/15 hover:to-violet-500/15 transition-all group cursor-pointer"
            >
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <p className="text-sm font-semibold text-white group-hover:text-cyan-400 transition-colors">Access Full Faculty Details on VIT Portal</p>
                        <p className="text-xs text-white/35">Opens the official VIT faculty information system in a new tab</p>
                    </div>
                </div>
                <ChevronRight size={16} className="text-white/30 group-hover:text-cyan-400 flex-shrink-0 transition-colors" />
            </motion.a>
            ...
            <div className="relative">
                ...
                <input
                    ...
                    className="w-full bg-white/[0.04] border border-white/[0.08] focus:border-cyan-500/40 rounded-2xl pl-12 pr-5 py-4 text-sm text-white placeholder-white/25 outline-none transition-colors focus:bg-white/[0.06]"
                />
                {query && (
            ...
            <div className="flex flex-wrap gap-2">
                {schools.map((s) => (
                    <button
                        ...
                        className={"px-4 py-1.5 rounded-full text-xs font-semibold transition-all " + (filter === s ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'bg-white/[0.03] text-white/40 border border-white/[0.07] hover:border-white/[0.15] hover:text-white/60')}
                    >
                        {s === 'all' ? 'All Schools' : s}
                    </button>
                ))}
            ...
            <div className="flex items-center text-sm">
                {loading ? (
                    <span className="flex items-center gap-1.5 text-white/40"><Loader2 size={12} className="animate-spin text-cyan-400" />Searching...</span>
                ) : (
                    <span className="text-white/40"><span className="text-cyan-400 font-semibold">{results.length}</span> {results.length === 1 ? 'faculty member' : 'faculty members'} found</span>
                )}
            </div>
            <AnimatePresence mode="wait">
                {results.length === 0 && !loading ? (
                    <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center py-20 text-center">
                        ...
                        <button onClick={() => handleSearch('')} className="mt-4 text-sm text-cyan-400 hover:text-cyan-300 transition-colors">Clear search</button>
                    </motion.div>
                ) : (
